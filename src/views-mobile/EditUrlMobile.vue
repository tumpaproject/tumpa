...
const url = ref(store.cardDetails?.public_key_url || '')
const adminPin = ref('')
...
async function save() {
  if (!url.value.trim() || !adminPin.value) {
    error.value = 'Please fill in all fields.'
    return
  }
  try {
    await setCardTransport(transport.value)
    await run('update_card_url', {
      url: url.value.trim(),
...
<template>
  <div class="form">
    <label class="label" for="card-url">Public URL</label>
    <input id="card-url" v-model="url" type="url" autocomplete="off" />

    <label class="label" for="card-url-pin">Current admin PIN</label>
    <input
      id="card-url-pin"
      v-model="adminPin"
      type="password"
